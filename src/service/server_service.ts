...
class ServerService {
	...
	async getServerDetails(serverId: string): Promise<IServer> {
		try {
			...
			if (!server)
				throw new EntityNotFound("User not found ${id}", "user");
			return server;
		} catch (err) {
			console.error(err);
	...
	async getConnectedServers(
		...
		try {
			...
			throw new ServerError(
				`Failed to fetch servers, for user : ${userId}`
			);
		}
	}
	...
	async getGuildsWhereUserIsOwner(
		...
		try {
			...
			const discordServersPreview =
				await this.discordService.getUserGuilds(accessToken, user._id);
